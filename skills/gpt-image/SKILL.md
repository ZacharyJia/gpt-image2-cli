...
7. **Execute via CLI only**: call `gpt-image` or `scripts/generate.py`. Do not create a new ad-hoc script for normal image requests.
...
# Repo-local Go build (when inside the checkout)
go run ./cmd/gpt-image -p "PROMPT" [options]

# Installed skill folder; use runtime-provided skill path when available
"$SKILL_DIR/scripts/generate.py" -p "PROMPT" [options]
```

`scripts/generate.py` is a launcher: repo-local `gpt-image` binary → PATH `gpt-image` → `go run ./cmd/gpt-image` fallback.
